...
export class VariableConfiguracionBonificacionDsService implements DataSource<VariableConfiguracion>{
  ...
  loadCVariableByBonificacion(columna: string, filter: string,
    sortDirection: string, pageIndex: number, pageSize: number) {

    this.loadingService.showLoading(true);
    this.loadingSubject.next(true);
    this.servVC.getVariableConfiguracionsCount("Bonificación");
    this.servVC.getVariableConfiguracionBonificacion(columna, filter, sortDirection, pageIndex, pageSize)
      .pipe(
        catchError(() => of([])),
        finalize(() => {
