...
export class VariableConfiguracionPagoDsService implements DataSource<VariableConfiguracion>{
  ...
  loadCVariableByPagos(columna: string, filter: string,
    sortDirection: string, pageIndex: number, pageSize: number) {

    this.loadingService.showLoading(true);
    this.loadingSubject.next(true);
    this.servVC.getVariableConfiguracionsCount("Pagos");
    this.servVC.getVariableConfiguracionPagos(columna, filter, sortDirection, pageIndex, pageSize)
      .pipe(
        catchError(() => of([])),
        finalize(() => {
