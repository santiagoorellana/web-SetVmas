...
export class VariableConfiguracionPuntosDsService implements DataSource<VariableConfiguracion>{
  ...
  loadCVariableByPuntos(columna: string, filter: string,
    sortDirection: string, pageIndex: number, pageSize: number) {

    this.loadingService.showLoading(true);
    this.loadingSubject.next(true);
    this.servVC.getVariableConfiguracionsCount("Puntos");
    this.servVC.getVariableConfiguracionPuntos(columna, filter, sortDirection, pageIndex, pageSize)
      .pipe(
        catchError(() => of([])),
        finalize(() => {
